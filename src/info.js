
    const info = {
        "arrLeft" : ["О продукте", "Отзывы", "Доставка и оплата", "Контакты"],
        "arrRight": [`<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M18.3337 1.6665L9.16699 10.8332" stroke="#2B3350" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    <path d="M18.3337 1.6665L12.5003 18.3332L9.16699 10.8332L1.66699 7.49984L18.3337 1.6665Z" stroke="#2B3350" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>  <span class = "textLeftElem"> Москва </span> `, `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <g clip-path="url(#clip0_68_408)">
    <path d="M7.50032 18.3334C7.96056 18.3334 8.33365 17.9603 8.33365 17.5001C8.33365 17.0398 7.96056 16.6667 7.50032 16.6667C7.04009 16.6667 6.66699 17.0398 6.66699 17.5001C6.66699 17.9603 7.04009 18.3334 7.50032 18.3334Z" stroke="#2B3350" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    <path d="M16.6663 18.3334C17.1266 18.3334 17.4997 17.9603 17.4997 17.5001C17.4997 17.0398 17.1266 16.6667 16.6663 16.6667C16.2061 16.6667 15.833 17.0398 15.833 17.5001C15.833 17.9603 16.2061 18.3334 16.6663 18.3334Z" stroke="#2B3350" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    <path d="M0.833008 0.833252H4.16634L6.39967 11.9916C6.47588 12.3752 6.6846 12.7199 6.9893 12.9652C7.29399 13.2105 7.67525 13.3407 8.06634 13.3333H16.1663C16.5574 13.3407 16.9387 13.2105 17.2434 12.9652C17.5481 12.7199 17.7568 12.3752 17.833 11.9916L19.1663 4.99992H4.99967" stroke="#2B3350" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    </g>
    <defs>
    <clipPath id="clip0_68_408">
    <rect width="20" height="20" fill="white"/>
    </clipPath>
    </defs>
    </svg> <span class = "textLeftElem"> Моя корзина </span> <div class= "purchases">+1</div>
    `],
    "textLogo" : 'Кресла Samurai',
    "textButton" : 'Заказать звонок',
    "tel": `+7 495 221-06-75`,
    "h2":`Кресла Samurai на выгодных условиях по всей стране`,
    "p" : `Здесь вам не придется покупать товар в слепую — проведите тест-драйв перед покупкой`,
    "textBtn" :  `Запись на тест-драйв`,
    stock:[{
        "srs":"/assets/images/Line.png",
        "text":"<p class = 'StockText'><strong>Бесплатная доставка</strong> по Москве и Санкт-Петербургу — от 3 шт.</p>",
        "textLittle":"Как получить?"
    },
    {
        "srs":"/assets/images/chess.png",
        "text":"<p class = 'StockText'><strong>Индивидуальные скидки</strong> при заказе от 3 шт.</p>",
        "textLittle":"Смотреть условия"
    },
    {
        "srs":"/assets/images/discount.png",
        "text":"<p class = 'StockText'> Для юридических лиц действуют <strong>особые условия</strong></p>",
        "textLittle":"Узнать подробнее"
    },], 
    "catalog":{
        textH3 :"Каталог кресел",
        filrerText:"По возрастанию цены",
        filter: [
            {
                text:"Спинка",
                option :[`Сетка`]
            },
            {
                text:"Сиденье",
                option :[`Кожзам`]
            },
            {
                text:"Подголовник",
                option :[`3D`]
            },
            {
                text:"Подлокотники",
                option :[`Все`]
            }
        ],
        armchair:[
            {
                id:"1",
                img:"/assets/images/arm.png",
                line:"/assets/images/LinePrice.png",
                name:"Samurai KL-1.04 Первое кресло",
                price:"23 000 ₽",
                oldPrice:"28 000 ₽",
                textBtn:"Добавить в корзину",
                description:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное отклонение спинки и сиденья кресла. Описание с первого кресла",
                mechanism:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное.",
                options:[
                    {
                        url:"/assets/images/black.png",
                        text:"Черный"
                
                    },
                    {
                        url:"/assets/images/blue.png",
                        text:"Синий"
                
                    },
                    {
                        url:"/assets/images/red.png",
                        text:"Красный"
                
                    }
                    ,
                    {
                        url:"/assets/images/green.png",
                        text:"Зеленый"
                
                    }
                ]
            },
            {
                id:"2",
                img:"/assets/images/arm.png",
                line:"/assets/images/LinePrice.png",
                name:"Samurai KL-1.04 Второе кресло",
                price:"23 000 ₽",
                oldPrice:"27 000 ₽",
                textBtn:"Добавить в корзину",
                description:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное отклонение спинки и сиденья кресла. Описание второго кресла",
                mechanism:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное.",
                options:[
                    {
                        url:"/assets/images/black.png",
                        text:"Черный"
                
                    },
                    {
                        url:"/assets/images/blue.png",
                        text:"Синий"
                
                    },
                    {
                        url:"/assets/images/red.png",
                        text:"Красный"
                
                    }
                    ,
                    {
                        url:"/assets/images/green.png",
                        text:"Зеленый"
                
                    }
                ]
            },
            {
                id:"3",
                img:"/assets/images/arm.png",
                line:"/assets/images/LinePrice.png",
                name:"Samurai KL-1.04 Кресло номер 3",
                price:"23 000 ₽",
                oldPrice:"27 000 ₽",
                textBtn:"Добавить в корзину",
                description:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное отклонение спинки и сиденья кресла.Описание Третего кресла",
                mechanism:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное.",
                options:[
                    {
                        url:"/assets/images/black.png",
                        text:"Черный"
                
                    },
                    {
                        url:"/assets/images/blue.png",
                        text:"Синий"
                
                    },
                    {
                        url:"/assets/images/red.png",
                        text:"Красный"
                
                    }
                    ,
                    {
                        url:"/assets/images/green.png",
                        text:"Зеленый"
                
                    }
                ]
            },
            {
                id:"4",
                img:"/assets/images/arm.png",
                line:"/assets/images/LinePrice.png",
                name:"Samurai KL-1.04 Четвертое кресло",
                price:"23 000 ₽",
                oldPrice:"27 000 ₽",
                textBtn:"Добавить в корзину",
                description:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное отклонение спинки и сиденья кресла. Описание кресла 4.",
                mechanism:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное.",
                options:[
                    {
                        url:"/assets/images/black.png",
                        text:"Черный"
                
                    },
                    {
                        url:"/assets/images/blue.png",
                        text:"Синий"
                
                    },
                    {
                        url:"/assets/images/red.png",
                        text:"Красный"
                
                    }
                    ,
                    {
                        url:"/assets/images/green.png",
                        text:"Зеленый"
                
                    }
                ]
            },
            {

                id:"5",
                img:"/assets/images/arm.png",
                line:"/assets/images/LinePrice.png",
                name:"Samurai KL-1.04 Крутое пятое кресло",
                price:"23 000 ₽",
                oldPrice:"27 000 ₽",
                textBtn:"Добавить в корзину",
                description:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное отклонение спинки и сиденья кресла. Описание пятого крутого кресла",
                mechanism:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное.",
                options:[
                    {
                        url:"/assets/images/black.png",
                        text:"Черный"
                
                    },
                    {
                        url:"/assets/images/blue.png",
                        text:"Синий"
                
                    },
                    {
                        url:"/assets/images/red.png",
                        text:"Красный"
                
                    }
                    ,
                    {
                        url:"/assets/images/green.png",
                        text:"Зеленый"
                
                    }
                ]
            },
             {
                id:"6",
                img:"/assets/images/arm.png",
                line:"/assets/images/LinePrice.png",
                name:"Samurai KL-1.04 кресло 6 ТОП за свои деньги",
                price:"23 000 ₽",
                oldPrice:"27 000 ₽",
                textBtn:"Добавить в корзину",
                description:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное отклонение спинки и сиденья кресла. Крутое кресло под номером 6.",
                mechanism:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное.",
                options:[
                    {
                        url:"/assets/images/black.png",
                        text:"Черный"
                
                    },
                    {
                        url:"/assets/images/blue.png",
                        text:"Синий"
                
                    },
                    {
                        url:"/assets/images/red.png",
                        text:"Красный"
                
                    }
                    ,
                    {
                        url:"/assets/images/green.png",
                        text:"Зеленый"
                
                    }
                ]
            }, 
            {
                id:"7",
                img:"/assets/images/arm.png",
                line:"/assets/images/LinePrice.png",
                name:"Samurai KL-1.04 777 супер-пупер",
                price:"23 000 ₽",
                oldPrice:"27 000 ₽",
                textBtn:"Добавить в корзину",
                description:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное отклонение спинки и сиденья кресла. Описание кресла 7777",
                mechanism:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное.",
                options:[
                    {
                        url:"/assets/images/black.png",
                        text:"Черный"
                
                    },
                    {
                        url:"/assets/images/blue.png",
                        text:"Синий"
                
                    },
                    {
                        url:"/assets/images/red.png",
                        text:"Красный"
                
                    }
                    ,
                    {
                        url:"/assets/images/green.png",
                        text:"Зеленый"
                
                    }
                ]
            },
            {
                id:"8",
                img:"/assets/images/arm.png",
                line:"/assets/images/LinePrice.png",
                name:"Samurai KL-1.04 8е кресло в списке",
                price:"27 000 ₽",
                oldPrice:"30 000 ₽",
                textBtn:"Добавить в корзину",
                description:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное отклонение спинки и сиденья кресла. Описание 8го кресла тут.",
                mechanism:"Эргономичное офисное кресло с синхромеханизмом качания «MultiBlock» — раздельное синхронное.",
                options:[
                    {
                        url:"/assets/images/black.png",
                        text:"Черный"
                
                    },
                    {
                        url:"/assets/images/blue.png",
                        text:"Синий"
                
                    },
                    {
                        url:"/assets/images/red.png",
                        text:"Красный"
                
                    }
                    ,
                    {
                        url:"/assets/images/green.png",
                        text:"Зеленый"
                
                    }
                ]
            }
        ],
        "ulCatalog":[
            {
                "text":"Главная",
                "url":"/"
            },
            {
                "text":"Каталог",
                "url":"/catalog"
            },
           

        ]
            
        
    },
    description:{
        h3:"Особенности кресел Самурай",
        p:"Здесь будет более подробно развернуто ключевое преимущество. Четвертое поколение Samurai, выпускаемое с сентября 2020 года. Исправлены десятки недочетов предыдущих поколений",
        arr:[
            {
               "h4":"Индивидуальные настройки",
               "text":"Учитывая ключевые сценарии поведения, высокое качество позиционных исследований способствует повышению"
            },
            {
                "h4":"Индивидуальные настройки",
                "text":"Учитывая ключевые сценарии поведения, высокое качество позиционных исследований способствует повышению"
             },
             {
                "h4":"Индивидуальные настройки",
                "text":"Учитывая ключевые сценарии поведения, высокое качество позиционных исследований способствует повышению"
             },
             {
                "h4":"Индивидуальные настройки",
                "text":"Учитывая ключевые сценарии поведения, высокое качество позиционных исследований способствует повышению"
             },
             {
                "h4":"Индивидуальные настройки",
                "text":"Учитывая ключевые сценарии поведения, высокое качество позиционных исследований способствует повышению"
             },
             {
                "h4":"Индивидуальные настройки",
                "text":"Учитывая ключевые сценарии поведения, высокое качество позиционных исследований способствует повышению"
             },
             {
                "h4":"Индивидуальные настройки",
                "text":"Учитывая ключевые сценарии поведения, высокое качество позиционных исследований способствует повышению"
             },
             {
                "h4":"Индивидуальные настройки",
                "text":"Учитывая ключевые сценарии поведения, высокое качество позиционных исследований способствует повышению"
             },
        ]
    },
    questions:{
        h4:"У вас остались вопросы?",
        text:"Оставьте контакты. Наш менеджер свяжется с вами и бесплатно проконсультирует",
        fildOne:"Ваше имя",
        fildTwo:"Телефон для связи",
        btn:"Заказать звонок"
    },
    footer:{
      arrLeft :  [`<div class="divLogoFooter">
      <svg width="184" height="30" viewBox="0 0 184 30" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      <rect x="37" y="6" width="147" height="22" fill="url(#pattern0)"></rect>
      <rect width="30" height="30" fill="url(#pattern1)"></rect><defs><pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1"><use xlink:href="#image0_68_415" transform="translate(-0.285714 -0.479545) scale(0.00442177 0.0295455)"></use></pattern><pattern id="pattern1" patternContentUnits="objectBoundingBox" width="1" height="1"><use xlink:href="#image0_68_415" transform="translate(-0.233333 -0.218333) scale(0.0216667)"></use></pattern> <image id="image0_68_415" width="300" height="94" xlink:href="data:image/png;base64,"></image></defs>
      </svg> <span class = "spanLogo"> Кресла Samurai</span>
  </div>
        `,
        `Политика конфиденциальности
        `,
        `© 2021 Все права защищены`,

    ],
    arrCentr : [
        `О продукте`,
        `Отзывы`,
        `Доставка и оплата`,
        `Контакты`
    ],
    arrRight : [
        `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M14.4183 5.49012C13.9423 5.40218 13.5051 5.70597 13.4144 6.17065C13.3238 6.63533 13.6286 7.08902 14.0916 7.17996C15.4859 7.45177 16.5625 8.53103 16.8354 9.93007C16.913 10.3328 17.2676 10.6266 17.6759 10.6266C17.7307 10.6266 17.7854 10.6216 17.8412 10.6116C18.3043 10.5187 18.6091 10.066 18.5184 9.6003C18.1111 7.51073 16.5027 5.89684 14.4183 5.49012Z" fill="#576077"/>
        <path d="M14.3561 2.00793C14.133 1.97595 13.9089 2.04191 13.7307 2.18381C13.5474 2.32771 13.4329 2.53557 13.408 2.76841C13.3552 3.23908 13.6948 3.66479 14.1649 3.71776C17.4065 4.07951 19.9262 6.60477 20.2907 9.85654C20.3395 10.2922 20.705 10.621 21.1412 10.621C21.174 10.621 21.2059 10.619 21.2388 10.615C21.4668 10.59 21.67 10.4771 21.8134 10.2972C21.9558 10.1174 22.0205 9.89351 21.9946 9.66467C21.5405 5.60746 18.4004 2.45862 14.3561 2.00793Z" fill="#576077"/>
        <path fill-rule="evenodd" clip-rule="evenodd" d="M11.0317 12.9724C15.0208 16.9604 15.9258 12.3467 18.4656 14.8848C20.9143 17.3328 22.3216 17.8232 19.2192 20.9247C18.8306 21.237 16.3616 24.9943 7.6846 16.3197C-0.993478 7.64402 2.76158 5.17246 3.07397 4.78397C6.18387 1.67386 6.66586 3.0894 9.11449 5.53734C11.6544 8.07651 7.04266 8.98443 11.0317 12.9724Z" fill="#576077"/>
        </svg> <span class = "telFooter"> +7 495 221-06-75</span>
        `,
        `<svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M16.939 3.69531C18.28 3.69531 19.57 4.22531 20.519 5.17631C21.469 6.12531 22 7.40531 22 8.74531V16.6453C22 19.4353 19.73 21.6953 16.939 21.6953H7.06C4.269 21.6953 2 19.4353 2 16.6453V8.74531C2 5.95531 4.259 3.69531 7.06 3.69531H16.939ZM18.07 8.89531C17.86 8.88431 17.66 8.95531 17.509 9.09531L13 12.6953C12.42 13.1763 11.589 13.1763 11 12.6953L6.5 9.09531C6.189 8.86531 5.759 8.89531 5.5 9.16531C5.23 9.43531 5.2 9.86531 5.429 10.1653L5.56 10.2953L10.11 13.8453C10.67 14.2853 11.349 14.5253 12.06 14.5253C12.769 14.5253 13.46 14.2853 14.019 13.8453L18.53 10.2353L18.61 10.1553C18.849 9.86531 18.849 9.44531 18.599 9.15531C18.46 9.00631 18.269 8.91531 18.07 8.89531Z" fill="#576077"/>
        </svg> <span> zakaz@kresla-samurai.ru</span>
        `,
        `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M17.9445 2.46193C16.357 0.874409 14.2462 0 12.0012 0C9.75577 0 7.64535 0.874409 6.05783 2.46193C3.11991 5.39942 2.7548 10.9264 5.26714 14.275L9.53464 20.4379C10.7277 22.1608 13.2746 22.1608 14.4676 20.4378L18.7251 14.2886C21.2476 10.9264 20.8825 5.39942 17.9445 2.46193ZM12.0788 11.3958C10.3874 11.3958 9.01066 10.0191 9.01066 8.3277C9.01066 6.6363 10.3874 5.2596 12.0788 5.2596C13.7702 5.2596 15.1469 6.6363 15.1469 8.3277C15.1469 10.0191 13.7702 11.3958 12.0788 11.3958Z" fill="#576077"/>
        </svg> <span> ул. Красного курсанта д.25, лит.в, оф. 114, бизнес-центр "RED CADET"</span>
        `,

    ]
    }

}


export  default info